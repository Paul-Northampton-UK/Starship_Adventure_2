âœ… Starship Adventure â€“ Detailed TODO List

ðŸ§± Core Engine

âœ… Set up folder structure: engine/, ui/, packs/, saves/, logs/, assets/
âœ… Create main.py entry point
âœ… Implement YAML loader (for rooms, objects, settings)
âœ… Design central game loop and state machine (GameState now robustly manages current_room_id, current_area_id, object states, and visibility logic)
âœ… Integrate spaCy for command parsing
âœ… Create alias/action translation system (e.g. n, go north, walk north) - Handled by NLP parser
âœ… Build modular command handler (actions, movement, examine, etc.) - Handlers created
âœ… Integrate loguru for dev/system logging
âœ… Implement loading of initial game state (start room, power state) from game_config.yaml (Basic loading done)

ðŸŽ¨ GUI & Interface

ðŸ”„ Build main game window with pygame
ðŸ”„ Add GUI components:
  - Room name header
  - Room exits footer
  - Main scrollable game output box
  - Narrator dialogue box
  - Visible objects box
  - Command input field (typed input)
  - Compass control (clickable directions)
  - Action buttons (Look, Take, Use, Inventory)
  - Toggleable side panels:
    - Inventory (weight/size shown)
    - Player stats (health, stamina, oxygen, etc.)
    - Environmental panel (oxygen, gravity, temperature)
    - Fog-of-war map panel
  - Score and move counter
  - Save / Load buttons
ðŸ”„ Implement UI theming/personalization options (future feature)

ðŸ§  Command & Interaction System

âœ… Support typed input with NLP variations (Basic NLP in place)
âœ… Enable command aliases and abbreviations (Handled by NLP patterns/verbs)
âœ… Refactor parser.py into smaller helper functions
ðŸ”„ Add player-to-narrator interactions (e.g. help, insult, ask, hint)
âœ… Handle invalid/gibberish commands with unique narrator responses (Implemented via responses.yaml)
âœ… Ensure command vocabulary is easily expandable via YAML (Objects/synonyms loaded)
âœ… Improve target extraction (handle prepositions like "with", "on", and also recognizes AREA entities as targets) - Improved via entity ruler and parser logic
ðŸ”„ Refine command parsing logic (e.g., disambiguation, error handling)
ðŸ”„ Implement profanity filtering for player input (Data files created, logic pending)
ðŸ”¸ Note: spaCy model `en_core_web_sm` must be installed as part of setup.
ðŸ”„ Implement fuzzy matching/typo tolerance (using fuzzywuzzy) - Potential future step
âœ… Implement response variations (using responses.yaml)
âœ… Handle plural items in responses
âœ… Display location description automatically on move
âœ… Parse two-word diagonal directions (e.g., "north west")
âœ… Add specific response for self-insertion container attempts (e.g., put pack in pack)
   - TODO: Implement 'put_fail_incomplete' response/logic.
   - TODO: Implement 'take_from_fail_incomplete' response/logic.
   - TODO: Refine 'take_from_fail_ambiguous' logic/response.
   - TODO: Refine wear/remove responses for different item types (e.g., "slip on" vs "wear" vs "put on").

ðŸŽ® Gameplay Mechanics

âœ… Define object structure (ID, name, description, size, weight, visibility, location, area_location, properties like is_takeable, is_openable_closable, etc.)
âœ… Build inventory management system (Core implemented: take, drop, wear, remove, put, take_from, wear_from; take/drop logic now correctly handles object visibility in rooms/areas)
âœ… Backpack system with size/weight constraints -> (Partially addressed by container logic, capacity check TODO)
âœ… Carried object tracking (In hand slot / worn items)
ðŸ”„ Implement environmental mechanics:
  - Room-specific oxygen, temperature, gravity, pressure
  - Hazards (e.g. suffocation, overheating, radiation)
  - Handle death scenarios and narrator warnings
âœ… Object container logic (Core implemented: put items in, take items out, wear items from held/worn containers)
   - TODO: Implement container capacity checks (size/weight/count).
   âœ… Implement container open/close states and check in commands (is_openable_closable property added, open/close handlers updated).
ðŸ”„ Enable object activation, assembly, disassembly
ðŸ”„ Implement HELP system (discuss approach: datapad vs command list)

ðŸ§© Puzzle & Story System

ðŸ”„ Implement puzzle system with difficulty tiers (Easy, Medium, Hard)
ðŸ”„ Add support for:
  - Logic puzzles
  - Code entry terminals
  - Physical object combination puzzles
  - Environmental / time-based puzzles
ðŸ”„ Implement scoring system based on puzzle difficulty
ðŸ”„ Track puzzle completion and tie it to story progress
ðŸ”„ Embed core story across room descriptions, tablets, terminals

ðŸ¤– Narrator System

ðŸ”„ Build narrator module:
  âœ… Contextual sarcasm and insults (Via responses.yaml)
  âœ… Humorous responses to player input (Via responses.yaml)
  ðŸ”„ Warnings before danger
  âœ… Custom reactions to swearing or nonsense input (Via responses.yaml, profanity logic pending)
  ðŸ”„ Narrator hint system with point penalty
  ðŸ”„ Narrator help system for tutorial commands

ðŸ’¾ Save/Load System

ðŸ”„ Implement SAVE/LOAD functionality
ðŸ”„ Auto-save on game exit
ðŸ”„ Manual save/load UI with multiple player profiles
ðŸ”„ Save system must persist:
  - Inventory
  - Rooms visited
  - Player stats
  - Puzzle progress
  - Environmental states

ðŸ”Š Audio/Visual

ðŸ”„ Add loading screen splash with fade + intro music
ðŸ”„ Implement ambient music engine (per area or event)
ðŸ”„ Add basic sound effects (UI actions, interactions, alerts)
ðŸ”„ Display optional static artwork per room or cutscene

ðŸ“œ Logging

âœ… Player log (all typed input + system output) - Captured by terminal
âœ… System log (functions triggered, errors, game state changes) - Implemented via logging module
âœ… Use loguru with rotating log files and timestamps - Basic logging setup done

ðŸ§© Expandability

âœ… Game world defined via external YAML
âœ… All rooms, objects, puzzles, and dialogue must be extendable
âœ… YAML validation and error catching (Via Pydantic schemas)
ðŸ”„ Document expansion format (README or DevDocs)

ðŸ“¦ Game Packs & Hub

ðŸ”„ Define pack structure under `packs/<pack_id>/` (rooms.yaml, objects.yaml, responses.yaml, profanity_words.yaml)
ðŸ”„ Implement pack loader: select `active_pack` and load its files into the engine
ðŸ”„ Update tools to save/edit per-pack files rather than root `packs/`
ðŸ”„ Hub app (`tools/main_design_hub.py`) to browse, create, validate, and launch packs

ðŸ§¹ Coding & Dev Standards

âœ… Follow naming conventions:
  - snake_case for variables/functions
  - PascalCase for classes
  - ID prefixes (e.g., nav_, com_)
âœ… Docstrings and inline comments throughout
âœ… Error handling for all I/O and YAML parsing (Basic handling implemented)
âœ… Clean modular codebase with reusable components (Ongoing effort)
   - TODO: Review necessity/usage of general GameState.inventory list.

ðŸ›¸ Future Features (Optional / Phase 2+)

ðŸ› ï¸ Admin Interface (Phase 1)

ðŸ”„ Core Setup:
- Create admin interface window structure
- Implement user authentication system
- Set up tab navigation system
- Design basic layout and styling

ðŸ”„ Object Management:
- Build object list panel with search/filter
- Create object editor panel
- Implement property editor with validation
- Add object relationship manager
- Build test panel for object interactions

ðŸ”„ Room Management:
- Create room list panel with filters
- Build room editor panel
- Implement grid-based room layout editor
- Add object placement tool
- Create exit/connection manager

ðŸ”„ Data Management:
- Implement auto-save system
- Add version history tracking
- Create backup/restore functionality
- Build export/import tools

ðŸ› ï¸ Admin Interface (Phase 2)

ðŸ”„ Puzzle Management:
  - Create puzzle list panel
  - Build puzzle editor interface
  - Implement solution step editor
  - Add hints management system
  - Create puzzle test environment

- Game Systems:
  - Build theme editor
  - Create audio management system
  - Implement loading screen editor
  - Add game settings manager

- Visual Tools:
  - Implement drag-and-drop interface
  - Create visual room layout editor
  - Build object placement grid
  - Add relationship visualization

ðŸ› ï¸ Admin Interface (Phase 3)

ðŸ”„ Testing Tools:
  - Create game state simulator
  - Build object interaction tester
  - Implement room transition tester
  - Add puzzle solution validator
  - Create performance metrics display

- Polish & Optimization:
  - Enhance UI/UX design
  - Optimize performance
  - Add advanced validation
  - Implement bulk edit tools
  - Create template system


