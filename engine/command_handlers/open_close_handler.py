...
def handle_open(game_state: GameState, parsed_intent: ParsedIntent) -> List[Dict[str, Any]]:
    ...
    logging.debug(f"[handle_open] Attempting to open: ID='{target_object_id}', Name='{target_object_name}'")

    if not target_object_id:
        logging.warning("[handle_open] No target object ID identified by parser.")
        return [{ "key": "OPEN_NO_TARGET", "data": {} }]

    obj_data = game_state.get_object_by_id(target_object_id)
    if not obj_data:
        logging.warning(f"[handle_open] Object data not found for ID: {target_object_id}")
    ...
    if not obj_props.get("is_openable_closable", False):
        logging.debug(f"[handle_open] Object '{target_object_name}' is not openable/closable based on properties.")
    ...
    if lock_details.get("locked", False): # Check runtime state
        logging.info(f"[handle_open] '{target_object_name}' is locked.")
        return [{"key": "OPEN_FAIL_LOCKED", "data": {"target_name": target_object_name}}]

    # Check if already open
    if obj_state.get("is_open", False):
        logging.info(f"[handle_open] '{target_object_name}' is already open.")
            ...
            if contained_item_references:
                for item_ref in contained_item_references:
                    ...
                    if isinstance(item_ref, str):
                        ...
                        logging.warning(f"[handle_open] Found a reference in container '{target_object_name}' of unexpected type or missing ID: {item_ref}")
                        continue

                    if not item_id_in_container: # Should be redundant if above logic is sound
                        logging.warning(f"[handle_open] Null or empty item_id_in_container from ref: {item_ref}")
                        continue
                    ...
                    if item_data_for_list:
                        ...
                        logging.warning(f"[handle_open] Could not retrieve data for item ID '{item_id_in_container}' found in '{target_object_name}'.")
                ...
                logging.info(f"[handle_open] '{target_object_name}' is empty according to its 'contains' state.")
                response_key = "OPEN_SUCCESS_EMPTY"
                response_data = {"container_name": target_object_name}
    ...
    logging.info(f"[handle_open] Successfully opened '{target_object_name}'. State: {obj_state}")
    ...
    if obj_props.get("is_storage", False):
        ...
        for item_ref in obj_state.get("contains", []) or []:
            ...
            if not item_id_in_container:
                logging.warning(f"[handle_open] Found an item_ref in container '{target_object_id}' without a valid ID: {item_ref}")
                continue
        ...
        if items_in_container_data:
            ...
            logging.info(f"[handle_open] '{target_object_name}' contains: {item_list_str}")
            ...
            logging.info(f"[handle_open] '{target_object_name}' is empty.")
...
def handle_close(game_state: GameState, parsed_intent: ParsedIntent) -> List[Dict[str, Any]]:
    ...
    logging.debug(f"[handle_close] Attempting to close: ID='{target_object_id}', Name='{target_object_name}'")

    if not target_object_id:
        logging.warning("[handle_close] No target object ID identified by parser.")
        return [{ "key": "CLOSE_NO_TARGET", "data": {} }]

    obj_data = game_state.get_object_by_id(target_object_id)
    if not obj_data:
        logging.warning(f"[handle_close] Object data not found for ID: {target_object_id}")
    ...
    if not obj_properties.get("is_openable_closable"):
        logging.info(f"[handle_close] Target '{obj_display_name}' cannot be closed.")
    ...
    if not is_open:
        logging.info(f"[handle_close] Target '{obj_display_name}' is already closed.")
        return [{ "key": "ALREADY_CLOSED", "data": {"target_name": obj_display_name} }]

    game_state.set_object_state(target_object_id, "is_open", False)
    logging.info(f"[handle_close] Successfully closed '{obj_display_name}'. State: {game_state.get_object_state(target_object_id)}")
